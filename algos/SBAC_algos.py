...
import os
import d4rl
from sample.Sample_from_dataset import ReplayBuffer
from sample.Prepare_env import prepare_env
...
class SBAC:
    ...
    def __init__(self, env_name,
        ...
        self.s_buffer, self.a_buffer, self.next_s_buffer, self.not_done_buffer, self.r_buffer, self.log_pi_buffer = [], [], [], [], [], []
    ...
    def online_fine_tune(self, total_time_step=1e+6):
        ...
        while i_so_far < total_time_step:
            ...
            for _ in range(10):
                state, action, next_state, reward, not_done, log_pi_last = self.sample_from_online_buffer(total_rollout_steps, self.batch_size)
                ...
                # action_pi = self.actor_net.get_action(state)
                state_norm = (state - self.s_mean) / (self.s_std + 1e-5)
                # log_pi_last = self.actor_last_net.get_log_density(state, action).detach()
                log_pi = self.actor_net.get_log_density(state, action)

                # log_miu = self.bc_standard_net.get_log_density(state_norm, action_pi)

                A = self.q_pi_net(state, action).detach()
                A = (A - A.mean())/(A.std() + 1e-5)
                ratio = torch.clip(log_pi-log_pi_last, min=-10, max=10)
                ratio = torch.exp(ratio)
                # assert torch.isinf(log_pi).sum() == 0, print(log_pi)
                # assert torch.isinf(log_pi_last).sum() == 0, print(log_pi_last)
                # assert torch.isinf(ratio).sum() == 0, print(ratio)
                L1 = ratio * A
                L2 = torch.clip(ratio, 0.8, 1.2) * A

                # policy update
                actor_loss = (-torch.min(L1, L2)).mean()
                self.actor_optim.zero_grad()
                # assert torch.isnan(self.actor_net).sum() == 0, print(self.actor_net.fc1)
                actor_loss.backward()
                self.actor_optim.step()
                # assert torch.isnan(self.actor_net.fc1).sum() == 0, print(self.actor_net.fc1)
    ...
    def rollout_online(self, rollout_length):
        ...
        while True:
            ...
            while True:
                ...
                action, log_pi, _ = self.actor_net(np.expand_dims(state, axis=0))
                action = action.cpu().detach().numpy().squeeze()
                log_pi = log_pi.cpu().detach().numpy().squeeze()
                log_pi_buffer.append(log_pi)
        ...
        self.log_pi_buffer = np.array(log_pi_buffer)
    ...
    def sample_from_online_buffer(self, total_rollout_steps, batch_size):
        ...
        return (
            ...
            torch.FloatTensor(self.log_pi_buffer[ind]).to(self.device)
        )
