        ...
        for t in range(int(total_time_step)):
            ...
            if t > self.batch_size:
                ...
                if t > self.start_steps:
                    ...
                    real_sa_prob = self.d_sa(s_train, a_train)
                    real_sas_prob = self.d_sas(s_train, a_train, next_s_train)
                    with torch.no_grad():
                        delta_r = torch.log(real_sas_prob + 1e-5) - torch.log(1. - real_sas_prob + 1e-5) - torch.log(real_sa_prob + 1e-5) + torch.log(1. - real_sa_prob + 1e-5)
