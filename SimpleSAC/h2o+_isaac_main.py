...
def main(argv):
    ...
    FLAGS.logging.output_dir = f'./experiment_output/standing-n'
    ...
    wandb.run.name = f"{FLAGS.env_name}_{FLAGS.file_path.split('/')[-1]}_seed={FLAGS.seed}_learnedDynamics={FLAGS.dynamics_model}_{FLAGS.current_time}"
        ...
        with Timer() as train_timer:
            for batch_idx in trange(FLAGS.n_train_step_per_epoch):
                ...
                if batch_idx + 1 == FLAGS.n_train_step_per_epoch:
                    metrics.update(
                        prefix_metrics(h2o.train(FLAGS.batch_size, 0), 'h2o')
                    )
                else:
                    h2o.train(FLAGS.batch_size, 0)
