...
FLAGS_DEF = define_flags_with_default(
    ...
    cql=DarcSAC.get_default_config(),
...
def main(argv):
    ...
    wandb.run.name = f"DARC_{FLAGS.name_str}_{FLAGS.env_list}_{FLAGS.data_source}_{FLAGS.unreal_dynamics}x{FLAGS.variety_list}_seed={FLAGS.seed}_{FLAGS.current_time}"
    ...
    sac = DarcSAC(FLAGS.cql, policy, qf1, qf2, target_qf1, target_qf2, d_sa, d_sas, replay_buffer)
        ...
        with Timer() as train_timer:
            for batch_idx in trange(FLAGS.n_train_step_per_epoch):
                ...
                if batch_idx + 1 == FLAGS.n_train_step_per_epoch:
                    metrics.update(
                        prefix_metrics(sac.train(sim_batch), 'sac')
                    )
                else:
                    sac.train(sim_batch)
