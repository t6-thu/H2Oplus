...
FLAGS_DEF = define_flags_with_default(
    ...
    h2o=Sim2realSAC.get_default_config(),
...
def main(argv):
    ...
    FLAGS.logging.output_dir = f'./experiment_output/standing-n-h2o'
    ...
    wandb.run.name = f"h2o_{FLAGS.env_name}_{FLAGS.file_path.split('/')[-1]}_seed={FLAGS.seed}_learnedDynamics={FLAGS.dynamics_model}_{FLAGS.current_time}"
    ...
    if FLAGS.dynamics_model:
        h2o = Sim2realSAC(FLAGS.h2o, policy, qf1, qf2, target_qf1, target_qf2, vf, replay_buffer,
                      dynamics_model=dynamics_model, dynamics_ratio_estimator=xi_sas)
    else:
        h2o = Sim2realSAC(FLAGS.h2o, policy, qf1, qf2, target_qf1, target_qf2, d_sa, d_sas, replay_buffer, dynamics_model=dynamics_model)
        ...
        with Timer() as train_timer:
            for batch_idx in trange(FLAGS.n_train_step_per_epoch):
                ...
                if batch_idx + 1 == FLAGS.n_train_step_per_epoch:
                    metrics.update(
                        prefix_metrics(h2o.train(real_batch_size, sim_batch_size), 'h2o')
                    )
                else:
                    h2o.train(real_batch_size, sim_batch_size)
